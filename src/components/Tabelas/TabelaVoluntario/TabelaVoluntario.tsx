...
import estilo from './TabelaVoluntario.module.css'; 
...
function TabelaVoluntario(): JSX.Element {
    ...
    useEffect(() => {
        const fetchVoluntarios = async () => {   // função para fazer a consulta de voluntarios
            try {
                ...
                console.error(`Erro ao buscar voluntarios: ${error}`); // Exibe erro no console se a requisição falhar
            }
        };
     ...
     * Por enquanto apenas exibe o ID no console
     */
    const editarVoluntario = (id: number) => {
        console.log(`Editar voluntário com ID: ${id}`);
        // Aqui você pode redirecionar para a página de edição
    };

    /**
     * Função que será chamada ao clicar no botão "Excluir"
     * Por enquanto apenas exibe o ID no console
     */
    const excluirVoluntario = (id: number) => {
        console.log(`Excluir voluntário com ID: ${id}`);
        // Aqui você pode chamar a API para excluir o voluntário
    };
    ...
    return (
        <>
            ...
            <main className={estilo['table-voluntario-container']}>
                {/* Botões de ação fora da tabela */}
                <div className={estilo['acoes-tabela']}>
                    <button className={estilo['adicionar']} onClick={() => window.location.href = '/voluntario/adicionar'}>Adicionar</button>
                    <button className={estilo['voltar']} onClick={() => window.location.href = '/tabelas'}>Voltar</button>
                </div>

                {/* Título da tabela com classe personalizada */}
                <h1 className={estilo['h1-titulo']}>Mural de Voluntários</h1>
                ...
                >
                    {/* Colunas da tabela */}
                    <Column field="nome" header="Nome" headerStyle={{ backgroundColor: '#FBED9B', color: '#000'}} style={{ width: '15%' }} />
                    <Column field="sobrenome" header="Sobrenome" headerStyle={{ backgroundColor: '#FBED9B', color: '#000'}} style={{ width: '15%' }} />
                    <Column field="endereco" header="Endereço" headerStyle={{ backgroundColor: '#FBED9B', color: '#000'}} style={{ width: '20%' }} />
                    <Column field="email" header="E-mail" headerStyle={{ backgroundColor: '#FBED9B', color: '#000'}} style={{ width: '20%' }} />
                    ...
                    <Column
                        ...
                        body={(rowData) => (
                            <div className={estilo['td-acoes']}>
                                <button className={estilo['editar']} onClick={() => editarVoluntario(rowData.id)}>Editar</button>
                                <button className={estilo['excluir']} onClick={() => excluirVoluntario(rowData.id)}>Excluir</button>
                            </div>
                        )}
                        style={{ width: '15%' }}
                    />
